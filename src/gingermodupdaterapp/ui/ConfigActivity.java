...
import gingermodupdaterapp.receiver.StartupReceiver;
...
public class ConfigActivity extends PreferenceActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (!Customization.Screenshotsupport) {
            Preference temp = findPreference(res.getString(R.string.PREF_CONFIG_EXTRAS));
            temp.setEnabled(false);
            temp.setSummary(R.string.config_extras_nosupport);
        }
        ...
        //Reset extras.extra
        pref = findPreference(res.getString(R.string.PREF_EXTRAS_EXTRA_FILE_DEF));
        pref.setOnPreferenceClickListener(new OnPreferenceClickListener() {
            public boolean onPreferenceClick(Preference preference) {
                prefs.setExtraFile(res.getString(R.string.conf_extra_version_file_def));
                Toast.makeText(getBaseContext(), R.string.p_extra_file_def_toast, Toast.LENGTH_LONG).show();
                Log.d(TAG, "Extra File Path set back to default: " + prefs.getExtraFile());
                ConfigActivity.this.finish();
                return true;
            }
        });
        ...
        //Display All Extra Updates
        pref = findPreference(res.getString(R.string.PREF_DISPLAY_OLDER_EXTRA_VERSIONS));
        pref.setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
            public boolean onPreferenceChange(Preference preference, Object newValue) {
                Toast.makeText(getBaseContext(), R.string.p_display_older_extra_versions_changed, Toast.LENGTH_LONG).show();
                return true;
            }
        });

        //Show Experimental Extras
        pref = findPreference(res.getString(R.string.PREF_DISPLAY_EXPERIMENTAL_EXTRA_VERSIONS));
        pref.setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
            public boolean onPreferenceChange(Preference preference, Object newValue) {
                Toast.makeText(getBaseContext(), R.string.p_allow_experimental_extra_versions_changed, Toast.LENGTH_LONG).show();
                return true;
            }
        });
        ...
        pref = findPreference(res.getString(R.string.PREF_EXTRAS_LIST));
        pref.setOnPreferenceClickListener(new OnPreferenceClickListener() {
            public boolean onPreferenceClick(Preference preference) {
                Intent i = new Intent(ConfigActivity.this, ExtraListActivity.class);
                startActivity(i);
                return true;
            }
        });
